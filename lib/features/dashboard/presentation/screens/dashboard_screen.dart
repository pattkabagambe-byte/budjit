...
class DashboardScreen extends ConsumerWidget {
      ...
      body: SafeArea(
        child: txAsync.when(
          ...
          data: (txs) {
            ...
            return _buildBody(
              context, ref, txs, recent, income, expenses, balance,
              savingsRate.toDouble(), score, currency, month,
              isDark, userId, budgetsAsync, goalsAsync,
            );
          },
        ),
      ),
    ...
    return CustomScrollView(
      ...
      slivers: [
        ...
        SliverToBoxAdapter(
          child: Padding(
            padding: const EdgeInsets.fromLTRB(20, 16, 20, 0),
            child: _QuickStats(income: income, expenses: expenses, currency: currency, txCount: txs.length),
        ...
        if (txs.any((t) => !t.isIncome))
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: _SpendingChart(txs: txs.where((t) => !t.isIncome).toList(), currency: currency, isDark: isDark),
        ...
        budgetsAsync.when(
          ...
          data: (budgets) => budgets.isEmpty
              ...
              : SliverToBoxAdapter(
                  child: Padding(
                    padding: const EdgeInsets.fromLTRB(20, 20, 20, 0),
                    child: _BudgetOverview(budgets: budgets, txs: txs, currency: currency, ref: ref),
        ...
        goalsAsync.when(
          ...
          data: (goals) => goals.isEmpty
              ...
              : SliverToBoxAdapter(
                  child: Padding(
                    padding: const EdgeInsets.fromLTRB(20, 20, 20, 0),
                    child: _GoalsPreview(goals: goals.take(3).toList(), currency: currency, ref: ref),
        ...
        if (recent.isEmpty)
          SliverToBoxAdapter(
            child: EmptyState(
              ...
              subtitle: 'Tap the button below to add your first income or expense.',
          ...
          SliverList(
            delegate: SliverChildBuilderDelegate(
              (_, i) => Padding(
                ...
                child: _TxTile(entry: recent[i], currency: currency, isDark: isDark),
  ...
  Widget _buildSkeleton() {
    return ListView(
      ...
      children: [
        const ShimmerBox(width: double.infinity, height: 200, borderRadius: BorderRadius.all(Radius.circular(24))),
        const SizedBox(height: 16),
        const ShimmerBox(width: double.infinity, height: 80, borderRadius: BorderRadius.all(Radius.circular(20))),
        const SizedBox(height: 16),
        Row(children: const [
          Expanded(child: ShimmerBox(width: double.infinity, height: 80, borderRadius: BorderRadius.all(Radius.circular(16)))),
          SizedBox(width: 12),
          Expanded(child: ShimmerBox(width: double.infinity, height: 80, borderRadius: BorderRadius.all(Radius.circular(16)))),
        ]),
      ],
        ...
        children: [
          ...
          if (balance < 0)
            const Text(
              ...
              style: TextStyle(color: AppColors.rose, fontSize: 12, fontWeight: FontWeight.w500),
            )
          else
            Text(
          ...
          Row(
            children: [
              Expanded(child: _MiniStat(label: 'Income', value: Fmt.compact(income, currency: currency), color: AppColors.emeraldLight, icon: Icons.arrow_downward_rounded)),
              const SizedBox(width: 12),
              Expanded(child: _MiniStat(label: 'Expenses', value: Fmt.compact(expenses, currency: currency), color: AppColors.coral, icon: Icons.arrow_upward_rounded)),
            ],
          ),
          if (income > 0) ...[
            const SizedBox(height: 16),
...
class _SafeToSpendCard extends StatelessWidget {
  ...
  const _SafeToSpendCard({required this.income, required this.expenses, required this.currency});
  ...
  Widget build(BuildContext context) {
    ...
    final safeToday = remaining > 0 && daysLeft > 0 ? remaining / daysLeft : 0.0;
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(Icons.today_rounded, color: AppColors.emerald, size: 22),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  safeToday > 0 ? Fmt.money(safeToday, currency: currency) : 'Over budget',
                  style: TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.w900,
                    color: safeToday > 0 ? (isDark ? Colors.white : AppColors.navy) : AppColors.rose,
                  ),
                ),
              ],
            ),
...
class _QuickStats extends StatelessWidget {
  ...
  const _QuickStats({required this.income, required this.expenses, required this.txCount, required this.currency});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            Expanded(child: _StatCard(label: 'Income', value: Fmt.compact(income, currency: currency), color: AppColors.emerald, icon: Icons.trending_up_rounded, isDark: isDark)),
            const SizedBox(width: 10),
            Expanded(child: _StatCard(label: 'Expenses', value: Fmt.compact(expenses, currency: currency), color: AppColors.rose, icon: Icons.trending_down_rounded, isDark: isDark)),
            const SizedBox(width: 10),
            Expanded(child: _StatCard(label: 'Entries', value: '$txCount', color: AppColors.violet, icon: Icons.receipt_long_rounded, isDark: isDark)),
          ],
        ),
      ],
    );
...
class _StatCard extends StatelessWidget {
  ...
  const _StatCard({required this.label, required this.value, required this.color, required this.icon, required this.isDark});
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(value, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w900, color: isDark ? Colors.white : AppColors.navy)),
...
class _SpendingChart extends StatelessWidget {
  ...
  const _SpendingChart({required this.txs, required this.currency, required this.isDark});
...
class _BudgetOverview extends StatelessWidget {
  ...
  const _BudgetOverview({required this.budgets, required this.txs, required this.currency, required this.ref});
        ...
        SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (_, i) {
              ...
              final ratio = b.limitAmount > 0 ? (spent / b.limitAmount).clamp(0.0, 1.0) : 0.0;
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            cat.label.split(' ').first,
                            style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: isDark ? Colors.white : AppColors.navy),
                            overflow: TextOverflow.ellipsis,
                          ),
                        ),
                      ],
...
class _GoalsPreview extends StatelessWidget {
  ...
  const _GoalsPreview({required this.goals, required this.currency, required this.ref});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        ...goals.map((g) {
          final ratio = g.targetAmount > 0 ? (g.currentAmount / g.targetAmount).clamp(0.0, 1.0) : 0.0;
          return Container(
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(g.name, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: isDark ? Colors.white : AppColors.navy)),
                      const SizedBox(height: 6),
                      ClipRRect(
                        borderRadius: BorderRadius.circular(999),
                        child: LinearProgressIndicator(
                ...
                Text(
                  ...
                  style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: AppColors.emerald),
                ),
              ],
            ),
          );
...
class _TxTile extends StatelessWidget {
  ...
  const _TxTile({required this.entry, required this.currency, required this.isDark});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          CategoryBadge(category: cat.id, emoji: cat.emoji, color: cat.color, size: 42),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(
                  entry.title,
                  style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: isDark ? Colors.white : AppColors.navy),
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                ),
                Text(
